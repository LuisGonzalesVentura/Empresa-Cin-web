...
export default function QRPagadosPage() {
  const {
    fecha,
    setFecha,
    loading,
  ...
  // Filtrar solo QRs con statusId === 2 (Usados)
  const qrsPagados = qrs.filter((qr) => qr.statusId === 2);

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">
        Consultar QRs pagados por fecha
      </h1>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        {/* Botón para volver a QRs generados */}
        <button
          onClick={() => router.push("/dashboard/anadir/qrs_generados")}
          className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded"
        >
          Volver a QRs Generados
        </button>
      </div>
      ...
      {qrsPagados.length > 0 ? (
        ...
        !loading && <p>No hay QRs pagados para la fecha seleccionada.</p>
      )}
    </div>
  );
}
