...
export default function ListadoProductos() {
  ...
  useEffect(() => {
    async function fetchProductosMerchandising() {
        try {
          const response = await fetch('/api/productos/merchandising');
          if (response.ok) {
            const data = await response.json();
            setProductos(data); // Guardamos los productos en el estado
          } else {
            setError('No se pudieron cargar los productos de merchandising.');
          }
        } catch (err) {
          console.error('Error al obtener productos de merchandising:', err);
          setError('Ocurrió un error al obtener los productos de merchandising.');
        } finally {
          setLoading(false);
        }
      }
  ...
  const handleSubmitMerchandising = async (e: React.FormEvent) => {
    ...
    formData.append('nombre_producto', nombreProducto); // Cambié el nombre del campo para coincidir con tu ejemplo de API
    formData.append('precio', precio.toString()); // Asegurarse de que el precio sea un string
    formData.append('descuento', descuento ? descuento.toString() : '0'); // Descuento predeterminado a '0' si está vacío
    formData.append('id_ciudad', ciudad.toString()); // ID de ciudad como string
    formData.append('foto', foto); // Adjuntar el archivo de foto
  
    try {
      const response = await fetch('/api/productos/agregar_merchandising', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        const data = await response.json();
        setProductos([...productos, data]); // Agregar el nuevo producto a la lista de productos
        setError(null); // Limpiar el mensaje de error
        setNombreProducto(''); // Limpiar los campos del formulario
        setPrecio('');
        setDescuento('');
        setCiudad('');
        setFoto(null);
        ...
        setError('Hubo un problema al insertar el producto de merchandising.');
      }
    } catch (err) {
      console.error('Error al insertar el producto de merchandising:', err);
      setError('Ocurrió un error al insertar el producto de merchandising.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white px-4 py-8 flex flex-col items-center">
      ...
      <div className="bg-white p-10 rounded-2xl shadow-xl w-full max-w-5xl mt-8">
        <h1 className="text-3xl font-bold text-orange-600 mb-6">Agregar Nuevo Merchadinsing</h1>
      ...
      {/* Tabla de productos */}
      <div className="bg-white p-10 rounded-2xl shadow-xl w-full max-w-5xl mt-8">
        <h1 className="text-3xl font-bold text-orange-600 mb-6">Listado de Merchadinsing</h1>
