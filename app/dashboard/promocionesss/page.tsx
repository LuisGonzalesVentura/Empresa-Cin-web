...
import { useSeleccionDepartamentoViewModel } from "../../../lib/promociones/useSeleccionDepartamentoViewModel";

export default function SeleccionProducto() {
  const { departamentos } = useSeleccionDepartamentoViewModel();
      ...
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-3xl text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {departamentos.map((dep) => (
            <Link
              key={dep.nombre}
              href={`/dashboard/promocionesss/${dep.nombre.toLowerCase()}`}
              passHref
            >
