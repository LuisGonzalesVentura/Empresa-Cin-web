...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    form.parse(req, async (err: Error | null, fields: Fields, files: Files) => {
      ...
      const { nombre_producto, precio, descuento, ciudad } = fields as unknown as ProductoFields;
      ...
      if (files.imagen && !Array.isArray(files.imagen)) {
        ...
        if (imagen?.filepath) {
          ...
          return res.status(400).json({ mensaje: 'La imagen no tiene una ruta válida.' });
        }
      }
      ...
      try {
        ...
        return res.status(500).json({ mensaje: 'Error desconocido al actualizar el producto' });
      }
    });
  } else {
